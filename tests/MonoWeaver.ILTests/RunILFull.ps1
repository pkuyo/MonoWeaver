...
if ([string]::IsNullOrWhiteSpace($TrxFileName)) {
    $TrxFileName = if ($hasExplicitFilter) { "il-full-filtered-results.trx" } else { "il-full-results.trx" }
}
...
try {
    ...
    $testArgs = @(
        ...
        "--filter", "FullyQualifiedName~FullMethodValidityMatchesExpectedResult",
    ...
    Write-Host "Running IL full verification tests..."
    if ($hasExplicitFilter) {
        Write-Host "ILTEST_FILTER=$env:ILTEST_FILTER"
    }
    else {
        Write-Host "ILTEST_FILTER cleared; running all full cases."
    }
